"""Tracking-gap diagnostic for Group B proxy ETFs in the Option C basket.

For each (proxy, reference) pair listed in OptionC.md Section 7.4, this script:
  - Loads the proxy series from data/processed/prices.parquet
  - Pulls the reference series from Yahoo Finance
  - Computes correlation, R-squared, beta, annualised tracking error,
    mean annual return differential, and annualised return / vol for each leg
  - Writes one row per pair to data/quality/tracking_gap_report.csv
  - Saves a scatter + cumulative-return overlay PNG per pair to data/quality/
...
PAIRS = [
    {"proxy_name": "Carbon", "proxy_symbol": "KRBN", "ref_symbol": "KEUA"},
    {"proxy_name": "Coal", "proxy_symbol": "COAL", "ref_symbol": "KOL"},
    {"proxy_name": "U_Phys", "proxy_symbol": "SRUUF", "ref_symbol": "URA"},
    {"proxy_name": "U_Min", "proxy_symbol": "URA", "ref_symbol": "URNM"},
]
...
def main() -> int:
    ...
    if not prices_path.exists():
        print(f"[ERROR] {prices_path} not found. Run the data-acquisition notebook first.",
              file=sys.stderr)
        return 1
